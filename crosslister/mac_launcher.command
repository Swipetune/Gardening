...
collect_suggestions() {
  ...
  case "$kind" in
    csv)
      results=$(find "$search_root" -maxdepth 4 -type f -name '*.csv' 2>/dev/null)
      ;;
    images_dir)
      results=$(find "$search_root" -maxdepth 4 -type d -mindepth 1 \( -name '*images*' -o -name '*photos*' -o -name '*media*' \) 2>/dev/null)
      ;;
    credentials)
      results=$(find "$search_root" -maxdepth 4 -type f -name '*credential*.json' 2>/dev/null)
      ;;
    cookies_dir)
      results=$(find "$HOME" -maxdepth 3 -type d -name '*Crosslister*' 2>/dev/null)
      ;;
    category_map)
      results=$(find "$search_root" -maxdepth 4 -type f -name '*category*map*.json' 2>/dev/null)
      ;;
    *)
      results=""
      ;;
  ...
  if [ -n "$default_path" ]; then
    results=$(printf '%s\n%s\n' "$default_path" "$results" | awk 'NF && !seen[$0]++' | head -n 9)
  else
    results=$(printf '%s\n' "$results" | awk 'NF && !seen[$0]++' | head -n 9)
  fi
...
choose_path() {
  ...
  while true; do
    suggestions=$(collect_suggestions "$suggestion_kind" "$default_value")
    options=()
    printf "\n%s%s%s\n" "$STYLE_BOLD" "$label" "$COLOR_RESET"
    printf "  %s0)%s Standaard gebruiken (%s)\n" "$COLOR_GREEN" "$COLOR_RESET" "$default_value"

    if [ -n "$suggestions" ]; then
      info_line "Beschikbare keuzes gevonden:"
      idx=1
      while IFS= read -r suggestion; do
        [ -z "$suggestion" ] && continue
        options+=("$suggestion")
        printf "  %s%d)%s %s\n" "$COLOR_GREEN" "$idx" "$COLOR_RESET" "$suggestion"
        idx=$((idx + 1))
      done <<EOF
$suggestions
EOF
    else
      info_line "Geen andere paden automatisch gevonden."
      info_line "Gebruik optie 0 voor het standaardpad of kies 'C' om zelf een pad in te voeren."
      if [ "$expected_kind" = "file" ]; then
        info_line "Tip: Kies 'C' en selecteer het juiste bestand als het standaardbestand ontbreekt."
      elif [ "$suggestion_kind" = "cookies_dir" ]; then
        info_line "Tip: Het standaardpad wordt automatisch aangemaakt voor cookies."
      else
        info_line "Tip: Kies 'C' om naar de map te bladeren als het standaardpad ontbreekt."
      fi
    fi

    if [ "$HAS_OSASCRIPT" -eq 1 ]; then
      if [ "$expected_kind" = "dir" ]; then
        info_line "Druk 'B' om via Finder een map te kiezen."
      else
        info_line "Druk 'B' om via Finder een bestand te kiezen."
      fi
    fi

    printf "  %sC)%s Eigen pad ingeven\n" "$COLOR_GREEN" "$COLOR_RESET"
    if [ "$HAS_OSASCRIPT" -eq 1 ]; then
      printf "  %sB)%s Bladeren in Finder\n" "$COLOR_GREEN" "$COLOR_RESET"
    fi

    choice=$(choose_from_menu "Kies optie [0]: " 0)
    case "$choice" in
      0)
        if validate_path "$default_value" "$expected_kind"; then
          echo "$default_value"
          return 0
        fi
        if [ "$suggestion_kind" = "cookies_dir" ]; then
          printf "%sStandaardmap niet gevonden; deze wordt aangemaakt.%s\n" "$COLOR_YELLOW" "$COLOR_RESET"
          echo "$default_value"
          return 0
        fi
        printf "%sStandaardpad bestaat niet. Kies een andere optie.%s\n" "$COLOR_YELLOW" "$COLOR_RESET"
        ;;
      [cC])
        printf "%sVoer een volledig pad in: %s" "$COLOR_GREEN" "$COLOR_RESET"
        read -r custom
        ...
        if validate_path "$custom" "$expected_kind"; then
          echo "$custom"
          return 0
        fi
        printf "%sPad bestaat niet of is van het verkeerde type.%s\n" "$COLOR_RED" "$COLOR_RESET"
        ;;
      [bB])
        if [ "$HAS_OSASCRIPT" -eq 0 ]; then
          printf "%sFinder-integratie is niet beschikbaar. Kies een andere optie.%s\n" "$COLOR_RED" "$COLOR_RESET"
          continue
        fi
        if [ "$expected_kind" = "dir" ]; then
          browse_script='POSIX path of (choose folder with prompt "Kies een map voor $label")'
        else
          browse_script='POSIX path of (choose file with prompt "Kies een bestand voor $label")'
        fi
        if ! selection=$(osascript -e "$browse_script" 2>/dev/null); then
          printf "%sGeen keuze gemaakt in Finder.%s\n" "$COLOR_YELLOW" "$COLOR_RESET"
          continue
        fi
        selection=$(expand_path "$selection")
        if validate_path "$selection" "$expected_kind"; then
          echo "$selection"
          return 0
        fi
        printf "%sPad uit Finder lijkt ongeldig, probeer opnieuw.%s\n" "$COLOR_RED" "$COLOR_RESET"
        ;;
      *)
        if [[ "$choice" =~ ^[0-9]+$ ]]; then
          if [ "$choice" -ge 1 ] && [ "$choice" -le "${#options[@]}" ]; then
            local picked="${options[$((choice - 1))]}"
            if validate_path "$picked" "$expected_kind"; then
              echo "$picked"
              return 0
            fi
            printf "%sGeselecteerde optie lijkt niet geldig. Probeer opnieuw.%s\n" "$COLOR_RED" "$COLOR_RESET"
          else
            printf "%sNummer buiten bereik. Probeer opnieuw.%s\n" "$COLOR_RED" "$COLOR_RESET"
          fi
        else
          printf "%sOnbekende keuze. Gebruik het nummer of 'c'.%s\n" "$COLOR_RED" "$COLOR_RESET"
        fi
        ;;
    esac
    options=()
...
CSV_PATH=$(choose_path "CSV-bestand" "$DEFAULT_CSV" csv file)
IMAGES_DIR=$(choose_path "Map met afbeeldingen" "$DEFAULT_IMAGES" images_dir dir)
CREDENTIALS_PATH=$(choose_path "Credentials-bestand" "$DEFAULT_CREDENTIALS" credentials file)
COOKIES_DIR=$(choose_path "Cookies-map" "$DEFAULT_COOKIES" cookies_dir dir)
CATEGORY_MAP_PATH=$(choose_path "Category-map (JSON)" "$DEFAULT_CATEGORY_MAP" category_map file)
